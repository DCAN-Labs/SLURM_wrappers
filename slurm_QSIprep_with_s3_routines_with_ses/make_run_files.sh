	...
	if [ "sub" = "${sub_text}" ]; then # if parsed text matches to "sub", continue
		...
		for j in `s3cmd ls ${data_bucket}/${sub_text}-${subj_id}/ | awk '{print $2}'`; do
			...
			if [ "ses" = "${ses_text}" ]; then
				...
				sed -e "s|SUBJECTID|${subj_id}|g" -e "s|SESID|${ses_id}|g" -e "s|DATADIR|${data_dir}|g" -e "s|BUCKET|${data_bucket}|g" -e "s|WORK_DIR|${work_dir}|g" -e "s|RUNDIR|${run_folder}|g" ${run_folder}/${QSIprep_template} > ${QSIprep_folder}/run${k}
				k=$((k+1))
			fi
		done
	fi
...
chmod 775 -R ${QSIprep_folder}

sed -e "s|GROUP|${group}|g" -e "s|EMAIL|${email}|g" -i ${run_folder}/resources_QSIprep_run.sh 
