...
data_dir="$2" # where to output data for nibabies 
xcpd_dir="$3" # where to output data for xcp-d
data_bucket="$1" # bucket that BIDS data will be pulled from and processed outputs will be pushed to
run_folder=`pwd`

nibabies_folder="${run_folder}/run_files.nibabies_full_low_resources"
nibabies_template="template.nibabies_full_run_low_resources"
...
sed -e "s|GROUP|${group}|g" -e "s|EMAIL|${email}|g" -i ${run_folder}/resources_nibabies_full_run_low_resources.sh 
