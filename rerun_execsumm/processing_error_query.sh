...
full_sync_ids=full_sync_ids.csv
...
elif grep -l "remote files to delete" ${most_recent_file_info}; then #repeat previously outlined steps for a different error message
	full_sync_errors=$( grep -l "remote files to delete" ${most_recent_file_info} )
	sub_id_full_sync_error=$( cat $full_sync_errors | grep "s3://*/" | cut -d/ -f6 | head -n 1 | sed 's/sub-//' )
	ses_id_full_sync_error=$( cat $full_sync_errors | grep "s3://*/" | cut -d/ -f7 | head -n 1 | sed 's/ses-//' | sed 's/ ...//' )
	sub_ses_id_full_sync_error="${sub_id_full_sync_error},${ses_id_full_sync_error}"
	echo $sub_ses_id_full_sync_error >> ${output_path}/${full_sync_ids}
else
...
full_sync_count=$(wc -w < full_sync_ids.csv)
echo "Total number of full_sync errors: $full_sync_count"
