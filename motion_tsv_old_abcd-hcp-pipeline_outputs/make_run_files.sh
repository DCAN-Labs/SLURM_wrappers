...
dicom2bids_template="template.reformat"
dicom2bids_subjects="${run_folder}/sub_files.dicom2bids"

bucket=s3://ABCC_year2
...
cat /home/rando149/shared/projects/ABCC_year2_motiontsv/processed_subjects.txt | while read line; do
	echo ${line} > ${dicom2bids_subjects}/sub-${line}.txt
	sed -e "s|SUBJECTID|${line}|g" -e "s|SUBJECTDIR|${dicom2bids_subjects}|g" -e "s|BUCKETPATH|${bucket}|g" -e "s|SESSIONID|${session}|g" ${run_folder}/template.reformat > ${dicom2bids_folder}/run${k}
