...
data_dir="/tmp/ABCC_year2_exec" # where to output data
bucket_name="s3://ABCC_year2_derivatives/processed/abcd-hcp-pipeline-v0.1.3" # bucket that BIDS data will be pulled from and processed outputs will be pushed to
run_folder="/home/faird/lundq163/projects/slurm_abcd-hcp-pipeline_rerun_part2/"
# ses_id="2YearFollowUpYArm1" #comment out if using csv
abcd_hcp_pipeline_folder="{run_folder}/run_files.abcd-hcp-pipeline_s3error_rerun".format(run_folder=run_folder)
...
with open(os.path.join(run_folder, 'sub_ids_without_executivesummary.csv')) as f:
    lines=f.readlines()
    bids_subjs = list()
    bids_ses = list()
    for line in lines:
...
for bids_subj in bids_subjs:
      ...
      os.system('sed -e "s|SUBJECTID|{sub_id}|g" -e "s|DATADIR|{data_dir}|g" -e "s|BUCKET|{bucket_name}|g" -e "s|RUNDIR|{run_folder}|g" {run_folder}/{abcd_hcp_pipeline_template} > {abcd_hcp_pipeline_folder}/run{k}'.format(k=count,abcd_hcp_pipeline_folder=abcd_hcp_pipeline_folder,run_folder=run_folder,sub_id=sub_id,data_dir=data_dir,abcd_hcp_pipeline_template=abcd_hcp_pipeline_template,bucket_name=bucket_name))
      count += 1      
'''
count=0
for bids_subj in bids_subjs:
