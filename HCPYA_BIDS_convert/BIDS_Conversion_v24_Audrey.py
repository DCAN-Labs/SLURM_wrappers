...
without creating new files. Note that the resulting 'link' directory will not be recognised as BIDS valid, but the 'copy'
version will.
...
'''
method='copy'
#Paths to the overall input and output directories are defined. Output is defined based on whether copying or linking.
...
with open(readme_filename, 'w') as f:
	f.write("""This data is taken from the HCP young adult dataset. It has been converted into BIDS
...
#A separate function is needed to convert the structural magnitude fieldmaps, as this involves turning
...
def magnitude_convert():
	...
	#Echoes 1 and 2 are defined as the first and second volumes according to the fourth dimension, respectively.
...
#This function is needed to convert BVEC and BVAL files for DWI scans. Separate function is needed as these are not NIFTI files.
def dwi_convert():
...
#This function merges multiple dictionaries together (as many as needed). Will be used to generate variants of metadata.
...
#These dictionaries contain BIDS metadata for the 3T and 7T scanners used. We'll be able to plug this in to all respective metadata.
...
def anat_metadata():
	...
	#The following parameters are specific to either T1 or T2, so they're defined separately. After doing this, the dictionary is merged with
...
def fmri_metadata():
	...
	#The following parameters are specific to either 3T or 7T, so they're defined separately. After doing this, the dictionary is merged with
	#hardware metadata and fields constant across scans in this category.
	if session == 'ses-3T':
...
#This function creates json metadata for diffusion scans.
def dwi_metadata():
	...
	for ev_file in glob.glob(os.path.join(EVs_filepath, '*.txt')):
		...
		with open(ev_file, 'r') as f:
			for line in f.readlines():
				...
				#Checks whether the 3 necessary rows (onset, duration, weight) are present.
	...
	for session in os.listdir(os.path.join(input_dir, subject)):
		...
		for protocol in os.listdir(os.path.join(input_dir, subject, session)):
			...
			#Finds diffusion scans.
				...
				#Diffusion scans are separated according to 'bvalue' (three for each session). This appropriate range of values is defined.
...
print("Script has successfully finished running.")
