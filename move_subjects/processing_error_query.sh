...
if grep -l "DUE TO TIME LIMIT" ${most_recent_file_info}; then #determine if error message in quotes is in most recent .err file
	...
	sub_id_time_limit=$( cat $time_limit_errors | grep "s3://*/" | cut -d/ -f6 | head -n 1 | sed 's/sub-//' ) #grab subject id info from the .err file
	ses_id_time_limit=$( cat $time_limit_errors | grep "s3://*/" | cut -d/ -f7 | head -n 1 | sed 's/ses-//' | sed 's/ ...//' ) #grab session id info from the .err file
	...
	sub_id_oom=$( cat $copy_errors | grep "s3://*/" | cut -d/ -f6 | head -n 1 | sed 's/sub-//' )
	ses_id_oom=$( cat $copy_errors | grep "s3://*/" | cut -d/ -f7 | head -n 1 | sed 's/ses-//' | sed 's/ ...//' )
	...
	sub_id_s3_error=$( cat $s3_errors | grep "s3://*/" | cut -d/ -f6 | head -n 1 | sed 's/sub-//' )
	ses_id_s3_error=$( cat $s3_errors | grep "s3://*/" | cut -d/ -f7 | head -n 1 | sed 's/ses-//' | sed 's/ ...//' )
	...
	sub_id_json_error=$( cat $json_errors | grep "s3://*/" | cut -d/ -f6 | head -n 1 | sed 's/sub-//' )
	ses_id_json_error=$( cat $json_errors | grep "s3://*/" | cut -d/ -f7 | head -n 1 | sed 's/ses-//' | sed 's/ ...//' )
	...
	sub_id_undetermined_error=$( cat $undetermined_errors | grep "s3://*/" | cut -d/ -f6 | head -n 1 | sed 's/sub-//' )
	ses_id_undetermined_error=$( cat $undetermined_errors | grep "s3://*/" | cut -d/ -f7 | head -n 1 | sed 's/ses-//' | sed 's/ ...//' )
